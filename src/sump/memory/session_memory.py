...
class SessionMemory:
    ...
    使用方式::
        ...
        mem = SessionMemory("data/session.db")
        mem.save_message("default", "user", "你好")
        messages = mem.load_messages("default", limit=50)
    """

    def __init__(self, db_path: str = "data/session.db") -> None:
    ...
    def delete_oldest_messages(self, session_id: str, count: int) -> int:
        """删除指定会话最旧的 count 条消息，返回实际删除条数。"""
        db = self._conn()
        try:
            cur = db.execute(
