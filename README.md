...
> 状态：M3 完成（记忆服务：完全启动链——经记忆激活后才调用 LLM API） · 设计文档：[DESIGN.md](./DESIGN.md)
...
```powershell
# 构建并启动三个服务（settings-center :9000 + memory :9201 + agentloop :9101）
.\scripts\run-local.ps1
```

- 自定义地址：`.\scripts\run-local.ps1 -CenterAddr 127.0.0.1:9000 -MemoryAddr 127.0.0.1:9201 -AgentAddr 127.0.0.1:9101`
- 记忆数据落在 `data/memory.db`（不入库）
